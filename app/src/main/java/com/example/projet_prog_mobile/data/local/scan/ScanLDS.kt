...
class ScanLDS @Inject constructor(
    ...
    private val scanStateFlow = MutableStateFlow<ScanModel?>(null)


    @OptIn(ExperimentalGetImage::class) override fun analyze(proxy: ImageProxy) {
        proxy.image?.let {
     ...
     fun getLatestScan(): Flow<ScanModel> = scanStateFlow
        .asStateFlow()
        .filterNotNull()
